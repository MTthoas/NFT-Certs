    ...
    useEffect(() => {
        if (
            ...
            if (idsArray.length !== urisArray.length) {
                console.error("Les tableaux tokenIds et tokenURIs n'ont pas la même longueur.")
                return
            }
    ...
    const mergedData = useMemo(() => {
        return certificationList.map((certification, index) => {
            ...
            const studentIdFromMetadata = metadata?.attributes?.find(
                (attr: { trait_type: string; value: any }) => attr.trait_type === 'studentId'
            )?.value

            const parentId = metadata?.attributes?.find(
                (attr: { trait_type: string; value: any }) => attr.trait_type === 'parentId'
            )?.value
        ...
        <UsersProvider>
            ...
            <Main>
                <div className='mb-2 flex flex-wrap items-center justify-between space-y-2'>
                    <h2 className='text-2xl font-bold tracking-tight'>Certification List</h2>
